...
TSHARK_EXCLUDE_PORTS          = os.getenv("TSHARK_EXCLUDE_PORTS", "").strip()
...
def build_tshark_command(interfaces : List[str], interface_by_api: bool) -> str:
    ...
    -x is required: json2pcap.py (Flow_Module) reconstructs a pcap from this
    JSON for CICFlowMeter using tshark's "frame_raw" field, which is only
    emitted when -x is given — without it every flow reconstruction fails
    silently and TAPCD never sees real flow features.
    """
    command = ["tshark"]
    ...
    if TSHARK_EXCLUDE_PORTS:
        ports = [p.strip() for p in TSHARK_EXCLUDE_PORTS.split(",") if p.strip()]
        capture_filter = " and ".join(f"not port {port}" for port in ports)
        command.extend(["-f", f"'{capture_filter}'"])
